...
typedef struct {
  int id; // ID real usado internamente, pero no se guarda en el archivo como
          // "00"
...
void agregarDispositivo() {
  ...
  while (!categoriaValida) {
    ...
    if (categoria >= 1 && categoria <= MAX_CATEGORIAS) {
      strcpy(dispositivos[contadorDispositivos].categoria,
  ...
  if (file) {
    fprintf(file, "00,%s,%s,%d,%s\n", dispositivos[contadorDispositivos].nombre,
            dispositivos[contadorDispositivos].categoria,
            dispositivos[contadorDispositivos].valor,
